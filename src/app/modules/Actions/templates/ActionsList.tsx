...
export const ActionsList: React.FC = () => {
    ...
    if (parentCategory) {
        breadcrumbsItems.push({
            ...
            link: `/actions/${parentCategory.id}`,
        })
    ...
    useEffect(() => {
        dispatch(categoriesActions.loadCategories('actions'))
        dispatch(documentsActions.loadDocuments('actions'))
    }, [])
    ...
    return (
        <>
            ...
            <Box pt={4} flex="1 0 100%" sx={{ overflow: 'auto', maxHeight: { md: 'calc( 100vh - 90px )' } }}>
                <CategoriesList type={'actions'} search={search} />
            </Box>
